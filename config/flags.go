...
import (
	...
	"github.com/hellobuild/Luv-Go/utils/ulimit"
...
func addNodeFlags(fs *flag.FlagSet) {
	...
	// LUV fees
	fs.Uint64(TxFeeKey, units.MilliLuv, "Transaction fee, in nLUV")
	fs.Uint64(CreationTxFeeKey, units.MilliLuv, "Transaction fee, in nLUV, for transactions that create new state")
	...
	fs.Uint64(MinValidatorStakeKey, 2*units.KiloLuv, "Minimum stake, in nLUV, required to validate the primary network")
	// Maximum Stake that can be staked and delegated to a validator on the Primary Network
	fs.Uint64(MaxValidatorStakeKey, 3*units.MegaLuv, "Maximum stake, in nLUV, that can be placed on a validator on the primary network")
	// Minimum Stake that can be delegated on the Primary Network
	fs.Uint64(MinDelegatorStakeKey, 25*units.Luv, "Minimum stake, in nLUV, that can be delegated on the primary network")
