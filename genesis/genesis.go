...
func validateInitialStakedFunds(config *Config) error {
	...
	for _, allocation := range config.Allocations {
		...
		allocationSet.Add(allocation.LUVAddr)
	}
...
func FromConfig(config *Config) ([]byte, ids.ID, error) {
	...
	}
	{
		...
		for _, allocation := range xAllocations {
			addr, err := formatting.FormatBech32(hrp, allocation.LUVAddr.Bytes())
			if err != nil {
				return nil, ids.ID{}, err
			}

			luv.InitialState["fixedCap"] = append(luv.InitialState["fixedCap"], avm.Holder{
		...
		var err error
		luv.Memo, err = formatting.Encode(defaultEncoding, memoBytes)
		if err != nil {
			return nil, ids.Empty, fmt.Errorf("couldn't parse memo bytes to string: %w", err)
		}
		avmArgs.GenesisData = map[string]avm.AssetDefinition{
			"LUV": luv, // The AVM starts out with one asset: LUV
		}
	}
	...
	avaxAssetID, err := LUVAssetID(bytes)
	if err != nil {
		return nil, ids.ID{}, fmt.Errorf("couldn't generate LUV asset ID: %w", err)
	}
	...
	for _, allocation := range config.Allocations {
		if initiallyStaked.Contains(allocation.LUVAddr) {
...
// LUVAssetID ...
func LUVAssetID(avmGenesisBytes []byte) (ids.ID, error) {
...
func (xa innerSortXAllocation) Less(i, j int) bool {
	return xa[i].InitialAmount < xa[j].InitialAmount ||
		(xa[i].InitialAmount == xa[j].InitialAmount &&
			bytes.Compare(xa[i].LUVAddr.Bytes(), xa[j].LUVAddr.Bytes()) == -1)
}
