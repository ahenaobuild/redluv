...
func (tx *UnsignedExportTx) SemanticVerify(
	...
	if err := tx.Verify(vm.ctx.XChainID, vm.ctx, vm.codec, vm.TxFee, vm.ctx.LUVAssetID); err != nil {
	...
	if err := vm.semanticVerifySpend(parentState, tx, tx.Ins, outs, stx.Creds, vm.TxFee, vm.ctx.LUVAssetID); err != nil {
		switch err.(type) {
		case permError:
	...
	// Produce the UTXOS
	txID := tx.ID()
	produceOutputs(newState, txID, vm.ctx.LUVAssetID, tx.Outs)
	return newState, nil
}
...
func (vm *VM) newExportTx(
	...
	return tx, utx.Verify(vm.ctx.XChainID, vm.ctx, vm.codec, vm.TxFee, vm.ctx.LUVAssetID)
}
