...
func (tx *UnsignedAddValidatorTx) Verify(
	...
	//add_validator_tx_luv.go function to check to whitelist governance for validators
	if err := tx.VerifyValidatorsWhitelist(ctx); err != nil {
		return err
	}
...
func (tx *UnsignedAddValidatorTx) SemanticVerify(
	...
	if vm.bootstrapped {
		...
		if err := vm.semanticVerifySpend(parentState, tx, tx.Ins, outs, stx.Creds, vm.AddStakerTxFee, vm.ctx.LUVAssetID); err != nil {
			switch err.(type) {
			case permError:
				return nil, nil, nil, nil, permError{
	...
	// Produce the UTXOS
	txID := tx.ID()
	produceOutputs(onCommitState, txID, vm.ctx.LUVAssetID, tx.Outs)
	...
	// Produce the UTXOS
	produceOutputs(onAbortState, txID, vm.ctx.LUVAssetID, outs)
