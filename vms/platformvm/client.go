...
import (
	...
	cjson "github.com/hellobuild/Luv-Go/utils/json"
	"github.com/hellobuild/Luv-Go/utils/rpc"
)
...
// ExportLUV issues an ExportLUV transaction and returns the txID
func (c *Client) ExportLUV(
	user api.UserPass,
	from []string,
	changeAddr string,
	to string,
	...
	res := &api.JSONTxID{}
	err := c.requester.SendRequest("exportLUV", &ExportLUVArgs{
		JSONSpendHeader: api.JSONSpendHeader{
			UserPass:       user,
			JSONFromAddrs:  api.JSONFromAddrs{From: from},
			JSONChangeAddr: api.JSONChangeAddr{ChangeAddr: changeAddr},
...
// ImportLUV issues an ImportLUV transaction and returns the txID
func (c *Client) ImportLUV(
	user api.UserPass,
	from []string,
	changeAddr,
	to,
	...
	res := &api.JSONTxID{}
	err := c.requester.SendRequest("importLUV", &ImportLUVArgs{
		JSONSpendHeader: api.JSONSpendHeader{
			UserPass:       user,
			JSONFromAddrs:  api.JSONFromAddrs{From: from},
			JSONChangeAddr: api.JSONChangeAddr{ChangeAddr: changeAddr},
...
// GetStake returns the amount of nLUV that [addresses] have cumulatively
...
// GetMinStake returns the minimum staking amount in nLUV for validators
...
// GetTotalStake returns the total amount (in nLUV) staked on the network
