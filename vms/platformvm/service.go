...
import (
	...
	"github.com/hellobuild/Luv-Go/utils/math"
	"github.com/hellobuild/redluv/utils/wrappers"
	"github.com/hellobuild/redluv/vms/avm"
	"github.com/hellobuild/redluv/vms/components/avax"
...
type GetBalanceResponse struct {
	// Balance, in nLUV, of the address
...
func (service *Service) CreateAddress(_ *http.Request, args *api.UserPass, response *api.JSONAddress) error {
	...
	if err := api.CheckGovernanceUsername(args.Username); err != nil {
		return err
	}
...
func (service *Service) GetStakingAssetID(_ *http.Request, args *GetStakingAssetIDArgs, response *GetStakingAssetIDResponse) error {
	...
	response.AssetID = service.vm.ctx.LUVAssetID
	return nil
}
...
// GetCurrentSupply returns an upper bound on the supply of LUV in the system
...
// ExportLUVArgs are the arguments to ExportLUV
type ExportLUVArgs struct {
	// User, password, from addrs, change addr
	api.JSONSpendHeader

	// Amount of LUV to send
	Amount json.Uint64 `json:"amount"`

	// ID of the address that will receive the LUV. This address includes the
	// chainID, which is used to determine what the destination chain is.
	To string `json:"to"`
}

// ExportLUV exports LUV from the P-Chain to the X-Chain
// It must be imported on the X-Chain to complete the transfer
func (service *Service) ExportLUV(_ *http.Request, args *ExportLUVArgs, response *api.JSONTxIDChangeAddr) error {
	service.vm.ctx.Log.Info("Platform: ExportLUV called")
...
// ImportLUVArgs are the arguments to ImportLUV
type ImportLUVArgs struct {
	// User, password, from addrs, change addr
	api.JSONSpendHeader
...
// ImportLUV issues a transaction to import LUV from the X-chain. The LUV
// must have already been exported from the X-Chain.
func (service *Service) ImportLUV(_ *http.Request, args *ImportLUVArgs, response *api.JSONTxIDChangeAddr) error {
	service.vm.ctx.Log.Info("Platform: ImportLUV called")
...
func (service *Service) getStakeHelper(tx *Tx, addrs ids.ShortSet) (uint64, []avax.TransferableOutput, error) {
	...
	for _, stake := range outs {
		// This output isn't LUV. Ignore.
		if stake.AssetID() != service.vm.ctx.LUVAssetID {
			continue
		}
		out := stake.Out
...
// GetStake returns the amount of nLUV that [args.Addresses] have cumulatively
// staked on the Primary Network.
//
// This method assumes that each stake output has only owner
// This method assumes only LUV can be staked
...
type GetMinStakeReply struct {
	...
	// Minimum stake, in nLUV, that can be delegated on the primary network
	MinDelegatorStake json.Uint64 `json:"minDelegatorStake"`
}

// GetMinStake returns the minimum staking amount in nLUV.
...
// GetMaxStakeAmount returns the maximum amount of nLUV staking to the named
