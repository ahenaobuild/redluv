...
func (tx *UnsignedCreateChainTx) SemanticVerify(
	...
	if err := tx.Verify(vm.ctx, vm.codec, vm.CreationTxFee, vm.ctx.LUVAssetID); err != nil {
	...
	if err := vm.semanticVerifySpend(vs, tx, tx.Ins, tx.Outs, baseTxCreds, vm.CreationTxFee, vm.ctx.LUVAssetID); err != nil {
		return nil, err
	}
	...
	// Produce the UTXOS
	txID := tx.ID()
	produceOutputs(vs, txID, vm.ctx.LUVAssetID, tx.Outs)
...
func (vm *VM) newCreateChainTx(
	...
	return tx, utx.Verify(vm.ctx, vm.codec, vm.CreationTxFee, vm.ctx.LUVAssetID)
}
