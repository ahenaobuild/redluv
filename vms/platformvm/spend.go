...
// - [fee] is the amount of LUV that should be burned
...
func (vm *VM) stake(
	...
	// Amount of LUV that has been staked
	amountStaked := uint64(0)

	// Consume locked UTXOs
	for _, utxo := range utxos {
		// If we have consumed more LUV than we are trying to stake, then we
		// have no need to consume more locked LUV
		if amountStaked >= amount {
			break
		}

		if assetID := utxo.AssetID(); assetID != vm.ctx.LUVAssetID {
			continue // We only care about staking LUV, so ignore other assets
		}
	...
	// Amount of LUV that has been burned
	amountBurned := uint64(0)

	for _, utxo := range utxos {
		// If we have consumed more LUV than we are trying to stake, and we
		// have burned more LUV then we need to, then we have no need to
		// consume more LUV
		...
		if assetID := utxo.AssetID(); assetID != vm.ctx.LUVAssetID {
			continue // We only care about burning LUV, so ignore other assets
		}
