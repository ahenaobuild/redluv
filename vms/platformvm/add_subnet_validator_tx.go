...
func (tx *UnsignedAddSubnetValidatorTx) SemanticVerify(
	...
	if vm.bootstrapped {
		...
		if err := vm.semanticVerifySpend(parentState, tx, tx.Ins, tx.Outs, baseTxCreds, vm.TxFee, vm.ctx.LUVAssetID); err != nil {
	...
	// Produce the UTXOS
	produceOutputs(onAbortState, txID, vm.ctx.LUVAssetID, tx.Outs)
