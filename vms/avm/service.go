...
import (
	...
	"github.com/hellobuild/redluv/vms/components/verify"
	"github.com/hellobuild/redluv/vms/nftfx"
	"github.com/hellobuild/redluv/vms/secp256k1fx"

	safemath "github.com/hellobuild/redluv/utils/math"
)
...
type ImportArgs struct {
	...
	// Address receiving the imported LUV
	To string `json:"to"`
}

// ImportLUV is a deprecated name for Import.
func (service *Service) ImportLUV(_ *http.Request, args *ImportArgs, reply *api.JSONTxID) error {
	return service.Import(nil, args, reply)
}

// Import imports an asset to this chain from the P/C-Chain.
// The LUV must have already been exported from the P/C-Chain.
...
// ExportLUVArgs are arguments for passing into ExportAVA requests
type ExportLUVArgs struct {
	// User, password, from addrs, change addr
	api.JSONSpendHeader
	// Amount of nLUV to send
	Amount json.Uint64 `json:"amount"`
...
// ExportLUV sends LUV from this chain to the address specified by [to].
// [to] specifies which chain to export funds to.
// After this tx is accepted, the LUV must be imported to the P/C chain with an importTx.
// Returns the ID of the newly created atomic transaction
func (service *Service) ExportLUV(_ *http.Request, args *ExportLUVArgs, reply *api.JSONTxIDChangeAddr) error {
	return service.Export(nil, &ExportArgs{
		ExportLUVArgs: *args,
		AssetID:       service.vm.ctx.LUVAssetID.String(),
	}, reply)
}
