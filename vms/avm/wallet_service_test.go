...
func setupWS(t *testing.T, isLUVAsset bool) ([]byte, *VM, *WalletService, *atomic.Memory, *Tx) {
...
// 3) The wallet service that wraps the VM
// 4) atomic memory to use in tests
func setupWSWithKeys(t *testing.T, isLUVAsset bool) ([]byte, *VM, *WalletService, *atomic.Memory, *Tx) {
	genesisBytes, vm, ws, m, tx := setupWS(t, isLUVAsset)
