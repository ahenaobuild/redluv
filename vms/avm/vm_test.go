...
import (
	...
	"github.com/hellobuild/Luv-Go/database/mockdb"
...
func NewContext(tb testing.TB) *snow.Context {
	...
	tx := GetLUVTxFromGenesisTest(genesisBytes, tb)

	ctx := snow.DefaultContextTest()
	ctx.NetworkID = networkID
	ctx.ChainID = chainID
	ctx.LUVAssetID = tx.ID()
...
func GetLUVTxFromGenesisTest(genesisBytes []byte, tb testing.TB) *Tx {
	return GetCreateTxFromGenesisTest(tb, genesisBytes, "LUV")
}
...
func NewTx(t *testing.T, genesisBytes []byte, vm *VM) *Tx {
	return NewTxWithAsset(t, genesisBytes, vm, "LUV")
}
...
func setupIssueTx(t testing.TB) (chan common.Message, *VM, *snow.Context, []*Tx) {
	...
	avaxTx := GetLUVTxFromGenesisTest(genesisBytes, t)
	key := keys[0]
