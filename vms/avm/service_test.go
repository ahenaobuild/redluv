...
import (
	...
	"github.com/hellobuild/redluv/utils/sampler"
...
func setup(t *testing.T, isLUVAsset bool) ([]byte, *VM, *Service, *atomic.Memory, *Tx) {
	...
	if isLUVAsset {
		...
		genesisTx = GetLUVTxFromGenesisTest(genesisBytes, t)
	} else {
...
// 3) The service that wraps the VM
// 4) atomic memory to use in tests
func setupWithKeys(t *testing.T, isLUVAsset bool) ([]byte, *VM, *Service, *atomic.Memory, *Tx) {
	genesisBytes, vm, s, m, tx := setup(t, isLUVAsset)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := s.Import(nil, args, reply); err != nil {
				t.Fatalf("Failed to import LUV due to %s", err)
			}
		})
	}
}
