...
import (
	...
	"github.com/hellobuild/Luv-Go/staking"
...
func TestPackerPackBytes(t *testing.T) {
	...
	expected := []byte("\x00\x00\x00\x04Luv")
	if !bytes.Equal(p.Bytes, expected) {
...
func TestPackerUnpackBytes(t *testing.T) {
	var (
		p           = Packer{Bytes: []byte("\x00\x00\x00\x04Luv")}
...
func TestPackerPackFixedByteSlices(t *testing.T) {
	...
	p.PackFixedByteSlices([][]byte{[]byte("Luv"), []byte("Evax")})
	...
	expected := []byte("\x00\x00\x00\x02LuvEvax")
	if !bytes.Equal(p.Bytes, expected) {
...
func TestPackerUnpackFixedByteSlices(t *testing.T) {
	var (
		p           = Packer{Bytes: []byte("\x00\x00\x00\x02LuvEvax")}
		actual      = p.UnpackFixedByteSlices(4)
		expected    = [][]byte{[]byte("Luv"), []byte("Evax")}
		expectedLen = 12
	)
