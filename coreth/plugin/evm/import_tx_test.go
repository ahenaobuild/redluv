...
func TestImportTxSemanticVerifyApricotPhase0(t *testing.T) {
	...
	if err := unsignedImportTx.Verify(vm.ctx.XChainID, vm.ctx, vm.txFee, vm.ctx.LUVAssetID, apricotRulesPhase0); err != nil {
		t.Fatal(err)
	}
...
func TestImportTxSemanticVerifyApricotPhase2(t *testing.T) {
	...
	if err := unsignedImportTx.Verify(vm.ctx.XChainID, vm.ctx, vm.txFee, vm.ctx.LUVAssetID, apricotRulesPhase2); err != nil {
		t.Fatal(err)
	}
