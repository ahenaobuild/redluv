...
func (vm *VM) newImportTx(
	...
	importedAVAXAmount := importedAmount[vm.ctx.LUVAssetID]
	outs := []EVMOutput{}
	...
	for assetID, amount := range importedAmount {
		...
		if assetID == vm.ctx.LUVAssetID || amount == 0 {
			continue
		}
		outs = append(outs, EVMOutput{
			Address: to,
...
func (tx *UnsignedImportTx) EVMStateTransfer(ctx *snow.Context, state *state.StateDB) error {
	for _, to := range tx.Outs {
		if to.AssetID == ctx.LUVAssetID {
			log.Debug("crosschain X->C", "addr", to.Address, "amount", to.Amount, "assetID", "LUV")
