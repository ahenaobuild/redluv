...
import (
	...
	"github.com/hellobuild/Luv-Go/coreth/params"
	...
	safemath "github.com/hellobuild/Luv-Go/utils/math"
...
func (tx *UnsignedExportTx) SemanticVerify(
	...
	if err := tx.Verify(vm.ctx.XChainID, vm.ctx, vm.txFee, vm.ctx.LUVAssetID, rules); err != nil {
	...
	fc.Produce(vm.ctx.LUVAssetID, vm.txFee)
...
func (vm *VM) newExportTx(
	...
	// burn AVAX
	ins, signers, err := vm.GetSpendableFunds(keys, vm.ctx.LUVAssetID, toBurn)
	if err != nil {
	...
	return tx, utx.Verify(vm.ctx.XChainID, vm.ctx, vm.txFee, vm.ctx.LUVAssetID, vm.currentRules())
}
...
func (tx *UnsignedExportTx) EVMStateTransfer(ctx *snow.Context, state *state.StateDB) error {
	...
	for _, from := range tx.Ins {
		if from.AssetID == ctx.LUVAssetID {
			log.Debug("crosschain C->X", "addr", from.Address, "amount", from.Amount, "assetID", "LUV")
